  ...
  return (
    <nav className="bg-[#131313]/60 backdrop-blur-xl flex justify-around items-center h-20 px-4 pb-4 fixed bottom-0 w-full z-50 shadow-[0_-10px_30px_rgba(0,0,0,0.4)]">
      <Link
        ...
        className={`flex flex-col items-center justify-center px-4 py-1 transition-all active:scale-90 ${
          isActive('/list')
            ...
            : 'text-[var(--color-on-surface-variant)] hover:text-[#fafafa]'
        }`}
      >
      ...
      <Link
        ...
        className={`flex flex-col items-center justify-center px-4 py-1 transition-all active:scale-90 ${
          isActive('/')
            ? 'text-[var(--color-primary)] bg-[var(--color-primary)]/10 rounded-xl'
            : 'text-[var(--color-on-surface-variant)] hover:text-[#fafafa]'
        }`}
      >
        <span className="material-symbols-outlined" style={{ fontVariationSettings: "'FILL' 1" }}>add_circle</span>
        <span className="font-label text-[10px] font-medium uppercase tracking-widest">Yeni Kayıt</span>
      </Link>
      <Link
        to="/settings"
        className={`flex flex-col items-center justify-center px-4 py-1 transition-all active:scale-90 ${
